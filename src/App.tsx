...
import { playDamageSound, playDiscoverySound, playSuccessSound } from './game/audio';
...
const defaultInput: InputState = { up: false, down: false, left: false, right: false, sprint: false };
...
export default function App() {
  ...
  useEffect(() => {
    const onKeyDown = (event: KeyboardEvent) => {
      ...
      if (event.code === 'KeyW' || event.code === 'ArrowUp') keysRef.current.up = true;
      if (event.code === 'KeyS' || event.code === 'ArrowDown') keysRef.current.down = true;
      ...
      if (event.code === 'ShiftLeft' || event.code === 'ShiftRight') keysRef.current.sprint = true;
      if (event.code === 'KeyR' && screen !== 'menu') restartLevel();
      if (event.code === 'Escape' && screen === 'playing') setScreen('paused');
    };

    const onKeyUp = (event: KeyboardEvent) => {
      if (event.code === 'KeyW' || event.code === 'ArrowUp') keysRef.current.up = false;
      if (event.code === 'KeyS' || event.code === 'ArrowDown') keysRef.current.down = false;
      ...
      if (event.code === 'ShiftLeft' || event.code === 'ShiftRight') keysRef.current.sprint = false;
    };
  ...
  useEffect(() => {
    ...
    const loop = (timestamp: number) => {
      ...
      const result = updateGame(gameRef.current, keysRef.current, TOTAL_LEVELS, copy, dt);
    ...
    <div className="app-shell" dir={isArabic ? 'rtl' : 'ltr'}>
      <header className="topbar compact-topbar">
        ...
        <div className="topbar-actions">
          <button className="ghost-button" onClick={screen === 'menu' ? startGame : restartLevel}>
      ...
      <main className="game-layout single-column-layout">
        <section className="game-stage card">
          <div className="game-stage-frame">
            <GameCanvas frame={frame} state={gameRef.current} language={language} />
